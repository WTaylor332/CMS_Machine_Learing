...
def testLoadedModel(model, train, xTest, yTest):
    ...
    # plot of scattered train and validation data
    print()
    yPredTrain = modelLoaded.predict(xTrain)
    plt.clf()
    fig, ax = plt.subplots(1, 2, figsize=(12,6), sharey=True)
    ax[0].axis('equal')
    ...
    ax[0].set_ylim(-15,15)
    ax[0].minorticks_on()
    ax[0].grid(which='both', alpha=0.9, c='#DDDDDD')
    ...
    ax[1].set_ylim(-15,15)
    ax[1].minorticks_on()
    ax[1].grid(which='both', alpha=0.9, c='#DDDDDD')
    plt.savefig(f'{model[:start[0]]}_True_vs_predicted_scatter_{model[start[0]+1:]}.png', dpi=1200)
    print('scatter plot made')
    ...
    fig, ax = plt.subplots(1, 2, figsize=(12,6), sharey=True)
    ax[0].axis('equal')
    heatmap, xEdges, yEdges = np.histogram2d(yTrain.flatten(), yPredTrain.flatten(), bins = 300)
    ax[0].imshow(heatmap.T, extent=[xEdges[0], xEdges[-1], yEdges[0], yEdges[-1]], origin='lower')
    line = np.array([-15, 15])
    ax[0].plot(line, line, color='black')
    ax[0].plot(line, line+max(line)*0.2, '--', c='orange')
    ax[0].plot(line, line-max(line)*0.2, '--', c='orange')
    ...
    ax[1].axis('equal')
    heatmap, xEdges, yEdges = np.histogram2d(yTest.flatten(), yPredicted.flatten(), bins = 300)
    ...
    ax[1].minorticks_on()
    ax[1].grid(which='both', alpha=0.9, c='#DDDDDD')
    plt.savefig(f'{model[:start[0]]}_True_vs_predicted_map_{model[start[0]+1:]}.png', dpi=1200)
    print('map plot made')
