...
def testLoadedModel(model, train, xTest, yTest):
    ...
    fig, ax = plt.subplots(1, 2, figsize=(12,6), sharey=True)
    ax[0].axis('equal')
    print(min(yTrain), max(yTrain), min(yPredTrain), max(yPredTrain))
    extent = np.array([[min(yTrain), max(yTrain)], [min(yPredTrain), max(yPredTrain)]])
    heatmap = ax[0].hist2d(yTrain, yPredTrain, bins=50, range=extent)
    ...
    heatmap = ax[1].hist2d(yTrain, yPredTrain, bins=50, range=extent)
